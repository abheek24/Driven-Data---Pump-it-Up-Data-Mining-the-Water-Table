...
library("flexclust")
cl1 = kcca(cbind(train$latitude, train$longitude), k=25, kccaFamily("kmeans"))
cl1
...
set.seed(123)
d <- sort(sample(nrow(train), nrow(train)*0.9))
dev<-train[d,]
val<-train[-d,]
...
library(dtplyr)
library(tidyverse)
rf = randomForest(status_group~. , dev[,-c(1,6,7)], mtry=4, ntree=150)
